...
const calculateCosts = () => {
    ...
    const venue = baseVenueRate * currentTierMultiplier * currentTypeMultiplier * currentVenueMultiplier;
    const catering = currentGuests * baseCateringPerPerson * currentTierMultiplier * currentFoodMultiplier;
    ...
    const animate = (timestamp) => {
        ...
        totalCostEl.innerText = formatCurrency(Math.floor(progress * (end - start) + start));
...
document.querySelectorAll('.venue-btn').forEach(btn => btn.addEventListener('click', (e) => {
...
document.querySelectorAll('.food-btn').forEach(btn => btn.addEventListener('click', (e) => {
...
document.querySelectorAll('.tier-card').forEach(card => card.addEventListener('click', (e) => {
    ...
    currentTierMultiplier = parseFloat(e.target.closest('.tier-card').dataset.tier);
    calculateCosts();
}));
...
document.getElementById('downloadBtn').addEventListener('click', () => {
    ...
    document.getElementById('pdfType').innerText = document.querySelector('.type-btn.active').innerText;
    document.getElementById('pdfVenueName').innerText = document.querySelector('.venue-btn.active').innerText; // NEW
    document.getElementById('pdfFoodName').innerText = document.querySelector('.food-btn.active').innerText; // NEW
    
    document.getElementById('pdfGuests').innerText = currentGuests;
    document.getElementById('pdfTier').innerText = document.querySelector('.tier-card.active h3').innerText;
    ...
    // Update Formulas
    document.getElementById('pdfVenueFormula').innerText = `Base: ${formatCurrency(baseVenueRate)} × ${currentTierMultiplier} (Tier) × ${currentTypeMultiplier} (Event) × ${currentVenueMultiplier} (Venue)`;
    document.getElementById('pdfCateringFormula').innerText = `Guests: ${currentGuests} × ₹${baseCateringPerPerson} × ${currentTierMultiplier} (Tier) × ${currentFoodMultiplier} (Food)`;
    document.getElementById('pdfDecorFormula').innerText = `Base: ${formatCurrency(baseDecorRate)} × ${currentTierMultiplier} (Tier) × ${currentTypeMultiplier} (Event)`;
